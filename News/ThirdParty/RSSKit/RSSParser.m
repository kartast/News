...
- (void)parseWithCompletionCallback:(RSSParserCompletionBlock) block {
    ...
    if (!success) {
        block(false, nil, [NSError errorWithDomain:@"unplug" code:-1 userInfo:nil]);
    }
}
...
- (void)parser:(NSXMLParser *)parser validationErrorOccurred:(NSError *)validationError {
    if (self.completionBlock) {
        self.completionBlock(self, nil, validationError);
    }
}
...
- (void) parser:(NSXMLParser *)parser parseErrorOccurred:(NSError *)error {
	...
	else if ([delegate respondsToSelector:@selector(rssParser:errorOccurred:)]) {
